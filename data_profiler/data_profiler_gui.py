...
from .frames.custom_widgets import StaticValueWithLabel

from .data_profiler import DataProfiler

from apex_gui.apex_app import ApexApp
from apex_gui.frames.styled_widgets import Page, PageContainer, Section, Frame, SectionHeaderLabel, PositiveButton, NegativeButton, IconButton
...
class DataProfilerGUI(ApexApp):
    ...
    def _create_home_frame(self, project_info: ExistingProjectProjectInfo):
        ...
        self.home_frame_project_name = EntryWithLabel(self.home_frame_project_info_frame, label_text='Project Name', default_val=project_info.project_name, entry_type=EntryType.String)
        self.home_frame_company = EntryWithLabel(self.home_frame_project_info_frame, label_text='Company', default_val=project_info.company_name, entry_type=EntryType.String)
        self.home_frame_company_location = EntryWithLabel(self.home_frame_project_info_frame, label_text='Location', default_val=project_info.company_location, entry_type=EntryType.String)
        self.home_frame_salesperson = EntryWithLabel(self.home_frame_project_info_frame, label_text='Salesperson', default_val=project_info.salesperson, entry_type=EntryType.String)
        self.home_frame_email = EntryWithLabel(self.home_frame_project_info_frame, label_text='Email', default_val=project_info.email, entry_type=EntryType.String)
        self.home_frame_start_date = EntryWithLabel(self.home_frame_project_info_frame, label_text='Start Date', default_val=project_info.start_date, entry_type=EntryType.String)
        self.home_frame_notes = EntryWithLabel(self.home_frame_project_info_frame, label_text='Notes', default_val=project_info.notes, entry_type=EntryType.String)
        # self.home_frame_project_name = CTkLabel(self.home_frame_project_info_frame, text=project_info.project_name)
        # self.home_frame_company = CTkLabel(self.home_frame_project_info_frame, text=project_info.company_name)
        # self.home_frame_company_location = CTkLabel(self.home_frame_project_info_frame, text=project_info.company_location)
        # self.home_frame_salesperson = CTkLabel(self.home_frame_project_info_frame, text=project_info.salesperson)
        # self.home_frame_email = CTkLabel(self.home_frame_project_info_frame, text=project_info.email)
        # self.home_frame_start_date = CTkLabel(self.home_frame_project_info_frame, text=project_info.start_date)
        # self.home_frame_notes = CTkLabel(self.home_frame_project_info_frame, text=project_info.notes)
        
        date_for_analysis = project_info.transform_options.date_for_analysis.value if project_info.transform_options.date_for_analysis else ''
        weekend_date_rule = project_info.transform_options.weekend_date_rule.value if project_info.transform_options.weekend_date_rule else ''

        self.home_frame_data_uploaded = StaticValueWithLabel(self.home_frame_data_info_frame, label_text='Data Uploaded', value=str(project_info.data_uploaded))
        self.home_frame_upload_date = StaticValueWithLabel(self.home_frame_data_info_frame, label_text='Upload Date', value=str(project_info.upload_date))
        ...
        self.home_frame_item_master_file = StaticValueWithLabel(self.home_frame_data_info_frame, label_text='Item Master File', value=str(project_info.uploaded_file_paths.item_master))
        self.home_frame_inbound_header_file = StaticValueWithLabel(self.home_frame_data_info_frame, label_text='Inbound Header File', value=str(project_info.uploaded_file_paths.inbound_header))
        self.home_frame_inbound_details_file = StaticValueWithLabel(self.home_frame_data_info_frame, label_text='Inbound Details File', value=str(project_info.uploaded_file_paths.inbound_details))
        self.home_frame_inventory_file = StaticValueWithLabel(self.home_frame_data_info_frame, label_text='Inventory File', value=str(project_info.uploaded_file_paths.inventory))
        self.home_frame_order_header_file = StaticValueWithLabel(self.home_frame_data_info_frame, label_text='Order Header File', value=str(project_info.uploaded_file_paths.order_header))
        self.home_frame_order_details_file = StaticValueWithLabel(self.home_frame_data_info_frame, label_text='Order Details File', value=str(project_info.uploaded_file_paths.order_details))
